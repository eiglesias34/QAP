...
class AntColony(QAP):
    ...
    def firstImprovement(self,ant,solLen,distance,flow):
        '''
        ...
        for i in range(self.number): 
            for j in range(i+1,self.number):
                newSol = ant.sol[:] 
                newSol[i],newSol[j] = ant.sol[j],ant.sol[i]
                newSolValue = ant.objectiveFunc(newSol,solLen,distance,flow)

                if newSolValue < ant.solValue:
                    return newSol,newSolValue

        return ant.sol,ant.solValue
    ...
    def colony(self, numIter):
        ...
        for x in range(numIter):
            ...
            for pos in range(self.populationSize):
                ...
                ant.sol, ant.solValue = self.firstImprovement(ant,self.number,self.distance,self.flow)
